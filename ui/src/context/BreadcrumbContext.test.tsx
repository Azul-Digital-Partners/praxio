...
describe("BreadcrumbContext", () => {
  ...
  it("builds page titles with the selected company name before Praxio", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "Anachronist Wiki")).toBe(
      "Inbox • Anachronist Wiki • Praxio",
    );
    expect(
      buildDocumentTitle(
    ...
    ).toBe("PAP-3515 • Issues • Anachronist Wiki • Praxio");
  });

  it("omits blank company names from page titles", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "  ")).toBe("Inbox • Praxio");
    expect(buildDocumentTitle([], null)).toBe("Praxio");
  });
});
